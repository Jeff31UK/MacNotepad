...
# Find all Swift source files (exclude generate_icon.swift)
SWIFT_FILES=$(find "$PROJECT_DIR/MacNotepad" -name "*.swift" -type f)
...
# Copy Info.plist
cp "$PROJECT_DIR/MacNotepad/App/Info.plist" "$CONTENTS/Info.plist"

# PkgInfo
echo -n "APPL????" > "$CONTENTS/PkgInfo"

# Generate app icon (.icns from PNGs)
ICON_DIR="$PROJECT_DIR/MacNotepad/Resources/Assets.xcassets/AppIcon.appiconset"
